...
def load_tscircuit(final):
    ...
    if not isinstance(data,list) or any(not isinstance(o,dict) or not isinstance(o.get('type'),str) for o in data):
        raise ValueError('circuit.json must be an array of typed objects')
...
def load_kicad(final):
    ...
    if len(netlists)==1:
        ...
        if raw.lstrip().startswith('<'):
            ...
            try:net=convert(ET.fromstring(raw))
            except ET.ParseError as exc:raise ValueError('Malformed XML netlist export') from exc
        else:net=sexpr(raw)
