...
def load_tscircuit(final):
    ...
    if len(files)!=1:raise ValueError('Expected exactly one circuit.json')
    ...
    f['board']={'width':w,'height':h,'layers':b['num_layers'],'bounds':[cx-w/2,cy-h/2,cx+w/2,cy+h/2]}
    f['schematic_present']=bool(get('schematic_component'));f['source_present']=bool(list(final.glob('*.tsx')))
...
def load_kicad(final):
    ...
    if len(boards)!=1:raise ValueError('Expected one native board')
    ...
    f['schematic_present']=len(schematics)==1 and sexpr(schematics[0].read_text())[0]=='kicad_sch'
    ...
    if not edgepts:raise ValueError('Unsupported or missing board outline')
    minx,miny,maxx,maxy=min(x for x,y in edgepts),min(y for x,y in edgepts),max(x for x,y in edgepts),max(y for x,y in edgepts)
    f['board']={'width':maxx-minx,'height':maxy-miny,'layers':len(copper_layers),'bounds':[minx,-maxy,maxx,-miny]}
