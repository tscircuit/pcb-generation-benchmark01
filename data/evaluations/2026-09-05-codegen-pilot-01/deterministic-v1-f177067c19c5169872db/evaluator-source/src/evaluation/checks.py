...
class Checker:
    def __init__(self,facts,rules,run_dir):
    ...
    def check(self,rule):
        ...
        if op=='board_size':return decision(abs(b['width']-rule['width_mm'])<=rule['tolerance_mm']+EPS and abs(b['height']-rule['height_mm'])<=rule['tolerance_mm']+EPS,{'width_mm':b['width'],'height_mm':b['height']})
