...
from src.evaluation.artifacts import load_kicad,load_tscircuit
from src.evaluation.checks import Checker
from src.evaluation.scoring import score,validate_rules
...
def main():
    ...
    if not args.experiment_id or Path(args.experiment_id).name!=args.experiment_id:raise ValueError('Invalid experiment ID')
    ...
    for item in config['execution_order']:
        ...
        checker=Checker(facts,rules,run_dir);outcomes=[]
