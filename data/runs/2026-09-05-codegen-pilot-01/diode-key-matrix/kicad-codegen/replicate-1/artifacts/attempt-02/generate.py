...
for r in range(1,4):
 for c in range(1,4):
  x=40.64+(c-1)*71.12;y=40.64+(r-1)*40.64
...
sym('Connector_Generic:Conn_01x06','J1','MATRIX',fpJ,50.8,165.1,[str(n) for n in range(1,7)])
for n,name in enumerate(['ROW1','ROW2','ROW3','COL1','COL2','COL3']):
 y=160.02+n*2.54;wire(45.72,y,30.48,y);label(name,30.48,y)
...
for c in range(1,4):
 x=6+20*(c-1);net=f'COL{c}';y=(7.5 if c==3 else 8+c);via(net,x,y)
...
for i,n in enumerate(['1:R1','2:R2','3:R3','4:C1','5:C2','6:C3']):text(n,20+i*2.54,1.7,.8)
text('J1: 1=ROW1 2=ROW2 3=ROW3',44,3,.8)
text('4=COL1 5=COL2 6=COL3',47,5,.8)
...
import shutil
symtab='(sym_lib_table (version 7)'
for l,n in [('Switch','SW_Push'),('Device','D'),('Connector_Generic','Conn_01x06')]:
 (OUT/(l+'.kicad_sym')).write_text('(kicad_symbol_lib (version 20250114)(generator "pcb-matrix-codegen")'+extract(l,n).replace('(symbol "'+l+':'+n+'"','(symbol "'+n+'"',1)+')')
 symtab+='(lib(name "'+l+'")(type "KiCad")(uri "${KIPRJMOD}/'+l+'.kicad_sym")(options "")(descr "Standard KiCad symbol subset"))'
(OUT/'sym-lib-table').write_text(symtab+')')
fptab='(fp_lib_table (version 7)'
for spec in [fpSW,fpD,fpJ]:
 l,n=spec.split(':');(OUT/(l+'.pretty')).mkdir(exist_ok=True)
 shutil.copy2(LIB/'footprints'/(l+'.pretty')/(n+'.kicad_mod'),OUT/(l+'.pretty')/(n+'.kicad_mod'))
 fptab+='(lib(name "'+l+'")(type "KiCad")(uri "${KIPRJMOD}/'+l+'.pretty")(options "")(descr "Standard KiCad footprint subset"))'
(OUT/'fp-lib-table').write_text(fptab+')')
