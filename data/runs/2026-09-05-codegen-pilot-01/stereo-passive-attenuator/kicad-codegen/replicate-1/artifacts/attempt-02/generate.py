...
for c in comps:
 x,y=c['sch'];ref=c['ref'];s=f'(symbol (lib_id "{c["sym"]}") (at {x} {y} 0) (unit 1) (in_bom {"no" if ref.startswith("TP") else "yes"}) (on_board yes) (dnp no) (uuid {c["uuid"]})'
...
for c in comps:
 lib,name=c['fp'].split(':');f=p.FootprintLoad(str(LIB/'footprints'/(lib+'.pretty')),name);lid=p.LIB_ID();lid.SetLibNickname(lib);lid.SetLibItemName(name);f.SetFPID(lid);f.SetReference(c['ref']);f.SetValue(c['val']);f.SetPosition(V(*c['pcb']));f.SetPath(p.KIID_PATH('/'+root+'/'+c['uuid']));b.Add(f);fps[c['ref']]=f
...
for x,suffix in [(109,'IN'),(140.5,'OUT')]:
 for y,txt in [(112.46,'1 LEFT_'+suffix),(115,'2 GND'),(117.54,'3 RIGHT_'+suffix)]:text(txt,x,y,.8)
text('STEREO AC ATTENUATOR',125,102,1);text('LEFT_OUT',141,101,.8);text('RIGHT_OUT',138,128,.8);text('GND',125,117,.8)
...
# Bundle exact standard symbols and footprints used, with local project tables.
import shutil
ld=D/'libraries';ld.mkdir(exist_ok=True)
for lib in sorted(set(c['sym'].split(':')[0] for c in comps)):
 syms=[get_symbol(s).replace('(symbol "'+s+'"','(symbol "'+s.split(':')[1]+'"',1) for s in sorted(set(c['sym'] for c in comps if c['sym'].split(':')[0]==lib))]
 (ld/(lib+'.kicad_sym')).write_text('(kicad_symbol_lib (version 20250114) (generator "kicad_symbol_editor") '+ '\n'.join(syms)+')')
for c in comps:
 lib,name=c['fp'].split(':');dest=ld/(lib+'.pretty');dest.mkdir(exist_ok=True);shutil.copy2(LIB/'footprints'/(lib+'.pretty')/(name+'.kicad_mod'),dest/(name+'.kicad_mod'))
for kind,key,ext in [('sym','sym','.kicad_sym'),('fp','fp','.pretty')]:
 libs=sorted(set(c[key].split(':')[0] for c in comps))
 (D/(kind+'-lib-table')).write_text('('+kind+'_lib_table\n'+ '\n'.join('(lib (name "'+lib+'") (type "KiCad") (uri "${KIPRJMOD}/libraries/'+lib+ext+'") (options "") (descr "Bundled standard KiCad library subset"))' for lib in libs)+')')
