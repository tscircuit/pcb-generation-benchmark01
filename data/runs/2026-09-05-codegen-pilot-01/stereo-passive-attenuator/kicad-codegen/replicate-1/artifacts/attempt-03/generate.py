...
for c in comps:
 lib,name=c['fp'].split(':');f=p.FootprintLoad(str(LIB/'footprints'/(lib+'.pretty')),name);f.SetFPID(p.LIB_ID(lib,name));f.SetReference(c['ref']);f.SetValue(c['val']);f.SetPosition(V(*c['pcb']));f.SetPath(p.KIID_PATH('/'+root+'/'+c['uuid']));b.Add(f);fps[c['ref']]=f
