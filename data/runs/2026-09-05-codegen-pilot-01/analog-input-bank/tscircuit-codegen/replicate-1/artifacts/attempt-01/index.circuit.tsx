...
export default () => <board width="80mm" height="50mm" layers={2}
  ...
  {[-36,36].flatMap(x=>[-21,21].map(y=><React.Fragment key={`${x},${y}`}>
    ...
    <pcbkeepout shape="circle" radius="2.6mm" pcbX={x} pcbY={y}/>
  </React.Fragment>))}
  {[1,2,3,4,5,6].map((i)=>{
    ...
    return <React.Fragment key={i}>
      ...
      <pinheader name={`J${i}`} pinCount={2} pitch="2.54mm" gender="male" showSilkscreenPinLabels pinLabels={[`IN${i}`,"GND"]} pcbX={x} pcbY={21} schX={sx-4} schY={sy} schSectionName={section}/>
  ...
  <pinheader name="J7" pinCount={8} pitch="2.54mm" gender="male" showSilkscreenPinLabels pinLabels={["OUT1","OUT2","OUT3","OUT4","OUT5","OUT6","GND","GND"]} pcbX={0} pcbY={-21} schX={42} schY={-12} schSectionName="Outputs"/>
