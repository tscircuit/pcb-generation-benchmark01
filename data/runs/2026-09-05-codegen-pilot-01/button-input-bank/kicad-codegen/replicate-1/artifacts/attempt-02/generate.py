...
trace('GND',[(12.54,4),(12.54,2),(47,2),(47,31),(8.65,31)])
for i in range(1,5):
 ...
 txt(n,x+3.25,21);txt(f'R{i} 10k',x+3,13.5,.8);txt(f'C{i} 100n',x+4,16.8,.8)
for j,n in enumerate(['1:+3V3','GND','BTN1','BTN2','BTN3','BTN4']):
 # individual signal labels below header, rotated for readability
 t=p.PCB_TEXT(b);t.SetText(n);t.SetPosition(V(10+2.54*j,8));t.SetTextAngle(p.EDA_ANGLE(90,p.DEGREES_T));t.SetTextSize(V(.8,.8));t.SetTextThickness(p.FromMM(.12));t.SetLayer(p.F_SilkS);b.Add(t)
...
(OUT/"sym-lib-table").write_text("(sym_lib_table\n"+"".join(f'(lib (name "{n}")(type "KiCad")(uri "{LIB}/symbols/{n}.kicad_sym")(options "")(descr ""))\n' for n in ["Device","Switch","Connector_Generic"])+")")
