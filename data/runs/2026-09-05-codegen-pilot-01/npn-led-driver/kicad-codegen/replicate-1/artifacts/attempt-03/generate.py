...
for ref,lib,sym,val,fp,(x,y),ns in parts:
 f=p.FootprintLoad(str(LIB/'footprints'/(fp.split(':')[0]+'.pretty')),fp.split(':')[1]);f.SetReference(ref);f.SetValue(val);f.SetPosition(p.VECTOR2I(p.FromMM(x),p.FromMM(y)));b.Add(f);fps[ref]=f;uuids[ref]=uid();path=p.KIID_PATH();path.push_back(p.KIID(root));path.push_back(p.KIID(uuids[ref]));f.SetPath(path);f.SetFPID(p.LIB_ID(fp.split(':')[0],fp.split(':')[1]))
...
for text,x,y,size in [('TWO CHANNEL LED DRIVER',73,77.5,1),('1 +5V',57.5,55.5,.8),('GND',57.5,58.2,.8),('CTRL1',57.5,63.08,.8),('CTRL2',57.5,66.5,.8)]:
...
def fillrect(layer,x0,y0,x1,y1,net):
 for ix in range(max(3,math.floor((x0-50)/step)),min(W-3,math.ceil((x1-50)/step)+1)):
  for iy in range(max(3,math.floor((y0-50)/step)),min(H-3,math.ceil((y1-50)/step)+1)):blocked[layer].setdefault((ix,iy),set()).add(net)
...
def free(x,y,l,n,via=False):
 ...
 if any(v!=n for v in blocked[l].get((x,y),set())):return False
 if via:
  px,py=xy((x,y))
  if any(math.hypot(px-54,py-hy)<1.0 for hy in [58,60.54,63.08,65.62]):return False
  for dx in range(-3,4):
   for dy in range(-3,4):
    if any(v!=n for v in blocked[l].get((x+dx,y+dy),set())):return False
 return True
...
def reserve(x,y,l,n,r=4):
 for dx in range(-r,r+1):
  for dy in range(-r,r+1):
   if dx*dx+dy*dy<=r*r:blocked[l].setdefault((x+dx,y+dy),set()).add(n)
...
for idx,(ref,lib,sym,val,fp,_,ns) in enumerate(parts):
 ...
 x=38.1+(idx%4)*50.8;y=38.1+(idx//4)*38.1
...
(OUT/'sym-lib-table').write_text('(sym_lib_table '+''.join('(lib (name "'+lib+'") (type "KiCad") (uri "'+str(LIB/'symbols'/(lib+'.kicad_sym'))+'") (options "") (descr ""))' for lib in ['Device','Connector_Generic','Transistor_BJT'])+')')
(OUT/'fp-lib-table').write_text('(fp_lib_table '+''.join('(lib (name "'+lib+'") (type "KiCad") (uri "'+str(LIB/'footprints'/(lib+'.pretty'))+'") (options "") (descr ""))' for lib in ['Resistor_SMD','Capacitor_SMD','LED_SMD','Connector_PinHeader_2.54mm','Package_TO_SOT_SMD'])+')')

# Local copies of the selected standard library primitives and footprints.
import shutil
local=OUT/'libraries';local.mkdir(exist_ok=True)
for lib in ['Device','Connector_Generic','Transistor_BJT']:
 names=sorted(set(part[2] for part in parts if part[1]==lib))
 (local/(lib+'.kicad_sym')).write_text('(kicad_symbol_lib (version 20241209) (generator "kicad_symbol_editor") '+''.join(getsym(lib,name)[0] for name in names)+')')
for fp in set(part[4] for part in parts):
 lib,name=fp.split(':');d=local/(lib+'.pretty');d.mkdir(exist_ok=True);shutil.copyfile(LIB/'footprints'/(lib+'.pretty')/(name+'.kicad_mod'),d/(name+'.kicad_mod'))
(OUT/'sym-lib-table').write_text('(sym_lib_table '+''.join('(lib (name "'+lib+'") (type "KiCad") (uri "${KIPRJMOD}/libraries/'+lib+'.kicad_sym") (options "") (descr ""))' for lib in ['Device','Connector_Generic','Transistor_BJT'])+')')
(OUT/'fp-lib-table').write_text('(fp_lib_table '+''.join('(lib (name "'+lib+'") (type "KiCad") (uri "${KIPRJMOD}/libraries/'+lib+'.pretty") (options "") (descr ""))' for lib in ['Resistor_SMD','Capacitor_SMD','LED_SMD','Connector_PinHeader_2.54mm','Package_TO_SOT_SMD'])+')')
