...
for i,y in enumerate([17,28,39,50],1):
 ...
 add('Q'+str(i),'AO3400A','Transistor_FET:Q_NMOS_GSD','Package_TO_SOT_SMD:SOT-23',['GATE'+str(i),'GND','OUT'+str(i)],59,y+2)
...
pads=[]
for c in components:
 lib,name=c['fp'].split(':');fp=p.FootprintLoad(str(LIB/'footprints'/f'{lib}.pretty'),name);fp.SetFPID(p.LIB_ID(lib,name));fp.SetReference(c['ref']);fp.SetValue(c['val']);fp.SetPosition(pt(c['x'],c['y']));fp.SetPath(p.KIID_PATH('/'+rootid+'/'+c['uuid']));board.Add(fp)
 ...
 for k,n in enumerate(c['nets']):
  if c['ref'].startswith('J'):
   t=p.PCB_TEXT(board);t.SetText(str(k+1)+':'+n);t.SetPosition(pt(27 if c['ref']=='J1' else 24 if c['ref']=='J2' else 75,c['y']+k*2.54 if c['ref'] in ['J1','J2'] else c['y']+5.2+k*1.6));t.SetTextSize(pt(.85,.85));t.SetTextThickness(mm(.12));t.SetLayer(p.F_SilkS);board.Add(t)
