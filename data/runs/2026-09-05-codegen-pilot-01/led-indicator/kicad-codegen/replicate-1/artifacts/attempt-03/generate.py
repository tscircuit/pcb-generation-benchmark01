...
for ref,libid,val,fp,*_ in parts:
 ...
 f.Reference().SetPosition(pcb.VECTOR2I(pcb.FromMM(x),pcb.FromMM(y-2.8)));f.Reference().SetTextAngle(pcb.EDA_ANGLE(0,pcb.DEGREES_T))
...
# Bundle exact installed library dependencies and register them locally.
import shutil
local=OUT/'libraries';local.mkdir(exist_ok=True)
for lib in ['Device','Connector_Generic']:
 shutil.copy2(LIB/'symbols'/(lib+'.kicad_sym'),local/(lib+'.kicad_sym'))
(OUT/'sym-lib-table').write_text('(sym_lib_table (version 7)\n'+''.join('(lib (name "'+lib+'") (type "KiCad") (uri "${KIPRJMOD}/libraries/'+lib+'.kicad_sym") (options "") (descr "Bundled KiCad 10 standard library"))\n' for lib in ['Device','Connector_Generic'])+')\n')
for _,_,_,fp,*_ in parts:
 lib,name=fp.split(':');dest=local/(lib+'.pretty');dest.mkdir(exist_ok=True)
 shutil.copy2(LIB/'footprints'/(lib+'.pretty')/(name+'.kicad_mod'),dest/(name+'.kicad_mod'))
(OUT/'fp-lib-table').write_text('(fp_lib_table (version 7)\n'+''.join('(lib (name "'+lib+'") (type "KiCad") (uri "${KIPRJMOD}/libraries/'+lib+'.pretty") (options "") (descr "Bundled KiCad 10 standard footprint"))\n' for lib in ['Connector_PinHeader_2.54mm','Resistor_SMD','LED_SMD'])+')\n')
