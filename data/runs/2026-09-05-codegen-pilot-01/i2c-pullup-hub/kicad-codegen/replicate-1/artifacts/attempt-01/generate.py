import pcbnew as p, os, re, uuid, json, math, heapq, time
...
parts.append(')')
for s in spec:
 x,y=s['sch'];parts.append(f'(symbol (lib_id "{s["lib"]}") (at {x} {y} 0) (unit 1) (in_bom yes) (on_board yes) (dnp no) (uuid "{s["uuid"]}")')
...
for s in spec:
 lib,fp=s['foot'].split(':');f=p.FootprintLoad(str(LIB/'footprints'/f'{lib}.pretty'),fp);f.SetReference(s['ref']);f.SetValue(s['val']);f.SetPosition(pt(*s['pos']));f.SetOrientationDegrees(s['rot']);f.SetPath(p.KIID_PATH('/'+root+'/'+s['uuid']))
...
for i,txt in enumerate(['1 3V3','2 GND','3 SDA','4 SCL']):text(txt,17+i*4,1.1,.65)
